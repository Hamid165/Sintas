...
<div class="space-y-6">
    <div class="flex justify-between items-center bg-white p-8 rounded-[2rem] border shadow-sm">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1 uppercase font-bold tracking-widest">SINTAS {{ date('d F Y') }}</p>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody id="trxList" class="divide-y text-sm">
            ...
            if(dKeu.length === 0) {
                ...
                trxList.innerHTML = dKeu.slice(0, 5).map((trx, idx) => `
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-300 font-black text-xs">${idx + 1}</td>
                        <td class="px-6 py-4 text-gray-500 font-bold text-xs whitespace-nowrap">${new Date(trx.created_at).toLocaleDateString('id-ID', {day:'numeric', month:'short', year:'numeric'})}</td>
                        <td class="px-6 py-4 font-black text-gray-800">${trx.kategori}</td>
                        <td class="px-6 py-4 text-gray-400 text-xs">${trx.keterangan || '-'}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase ${trx.jenis_transaksi === 'Pemasukan' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}">
                                <i data-lucide="${trx.jenis_transaksi === 'Pemasukan' ? 'arrow-down-left' : 'arrow-up-right'}" size="10"></i>
                                ${trx.jenis_transaksi}
                            </span>
                        </td>
