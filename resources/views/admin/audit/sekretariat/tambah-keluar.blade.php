...
@section('title', 'Surat Keluar - CareHub')
...
<div class="space-y-6 w-full">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.audit.sekretariat') }}" class="flex items-center gap-2 text-gray-400 hover:text-blue-600 transition-colors font-black text-xs uppercase tracking-widest">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-10 rounded-[2rem] text-white flex items-center justify-between">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 id="formTitle" class="text-2xl font-black uppercase tracking-tighter">Tambah Surat Keluar</h2>
                <p class="text-blue-100 text-xs font-bold uppercase tracking-widest mt-1">Rekap Kesekretariatan</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-[2rem] border-0 shadow-sm p-10">
        <form id="formSuratKeluar" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="kode_surat" placeholder="Contoh: SK-APR-2026-001"
                    class="w-full p-4 bg-gray-50 border-0 rounded-2xl font-bold text-gray-800 outline-none focus:ring-4 focus:ring-blue-100 transition-all text-sm" required>
                <p class="text-[10px] text-gray-400 ml-1">Harus unik. Format: SK-BLN-TAHUN-URUTAN</p>
            </div>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Tujuan <span class="text-rose-500">*</span></label>
                <input type="text" id="tujuan" placeholder="Nama instansi / organisasi tujuan"
                    class="w-full p-4 bg-gray-50 border-0 rounded-2xl font-bold text-gray-800 outline-none focus:ring-4 focus:ring-blue-100 transition-all text-sm" required>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="date" id="tanggal_surat"
                        class="w-full p-4 bg-gray-50 border-0 rounded-2xl font-bold text-gray-800 outline-none focus:ring-4 focus:ring-blue-100 transition-all" required>
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Tanggal Dikirim <span class="text-rose-500">*</span></label>
                    <input type="date" id="tanggal_dikirim"
                        class="w-full p-4 bg-gray-50 border-0 rounded-2xl font-bold text-gray-800 outline-none focus:ring-4 focus:ring-blue-100 transition-all" required>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (editId) {
        document.getElementById('formTitle').innerText = 'Edit Surat Keluar';
        document.getElementById('btnText').innerText = 'Simpan Perubahan';
        try {
            const res = await fetch(`/api/surat-keluar/${editId}`, {
            ...
            document.getElementById('tanggal_dikirim').value = data.tanggal_dikirim || '';
            document.getElementById('keterangan').value = data.keterangan || '';
        } catch(e) { showToast('Gagal memuat data surat.', 'error'); }
    }
});

document.getElementById('formSuratKeluar').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    const payload = {
        ...
        tanggal_dikirim: document.getElementById('tanggal_dikirim').value,
        keterangan: document.getElementById('keterangan').value,
    };

    const method = editId ? 'PUT' : 'POST';
    const url = editId ? `/api/surat-keluar/${editId}` : '/api/surat-keluar';
    ...
    try {
        ...
        if (res.ok) {
            const msg = editId ? 'Surat keluar berhasil diperbarui!' : 'Surat keluar berhasil ditambahkan!';
